...
def transcrever_video(id_video):
    ...
    return " ".join([trecho.text for trecho in transcrito.snippets]), False
    ...
    for i, video in enumerate(videos):
        ...
        try:
            ...
            if transcricao is None:
                print("\nParando execução: primeiro vídeo sem transcrição encontrado")
                print(f"Vídeo problemático: {video['id_video']}\n")
                break
...
def init():
    ...
    videos, faltantes = adicionar_transcricoes(videos, max_vds= random.randint(10,17))
